...
class Viewfinder(StreamParser):
    def __init__(self, filename, config):
        ...
        self.frames = [
            frame
            for _, frame in self.sample_frames(num_samples=frames_to_sample)
        ]
    ...
    def detect_screen(self):
        ...
        top = int(
            round(np.mean(pct_locations, axis=0)[0] - c.PERCENT_Y_POS * scale)
        )
        ...
        if actual_width < estimated_width:
            LOGGER.warning(
                "Estimated screen is bigger than the video, "
                "assuming left = 0..."
            )
            left = 0
        else:
    ...
    def get_pct_locations(self, scales):
        ...
        scale, pct_locations = self.locate(
            self.frames, PERCENT, matcher=matcher
        )

        if not scale or not pct_locations:
            raise RuntimeError(
                "This doesn't appear to be Melee (no percent signs found!)"
            )
    ...
    def detect_ports(self, scale, screen, max_error=0.06):
        ...
        for port_number in range(4):
            pct_roi = screen.subregion(
                c.PERCENT_Y_POS / c.SCREEN_HEIGHT,
                (c.PERCENT_X_POS + port_number * c.PERCENT_X_POS_STEP)
                / c.SCREEN_WIDTH,
            ...
            matcher = TemplateMatcher(
                scales=[scale], worst_match=0.6, debug=False
            )
            portscale, location = self.locate(
                self.frames, PERCENT, matcher=matcher, roi=pct_roi
            )
            ...
            if portscale is None or not location:
                ...
                LOGGER.warning(
                    "Detected port %d at (%d, %d) (error %.03f px, %.03f px)",
                    port_number + 1,
                    location[0],
                    location[1],
                    location[0] - pct_top,
                ...
                port_roi = screen.subregion(
                    c.PORT_Y_POS / c.SCREEN_HEIGHT,
                    (c.PORT_X_POS + port_number * c.PORT_X_POS_STEP)
                    / c.SCREEN_WIDTH,
